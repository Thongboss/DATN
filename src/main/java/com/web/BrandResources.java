...
public class BrandResources {
    ...
    public ResponseDto getAll(Pageable page) {
        return ResponseDto.of(this.brandService.findAll(page).map(b-> BrandDto.toDto(b, false)), "Get all brands");
    }

    @GetMapping("{id}")
    public ResponseDto getBrand(@PathVariable long id) {
        return ResponseDto.of(BrandDto.toDto(this.brandService.findById(id), false), "Get brand id: " + id);
    }

    @PostMapping
    private ResponseDto createBrand(@RequestBody BrandModel model) {
        model.setBrandId(null);
        return ResponseDto.of(BrandDto.toDto(this.brandService.add(model), false), "Add brand");
    }

    @PutMapping("{id}")
    public ResponseDto updateBrand(@PathVariable long id, @RequestBody BrandModel model) {
        model.setBrandId(id);
        return ResponseDto.of(BrandDto.toDto(this.brandService.update(model), false), "Update brand id: " + id);
    }
